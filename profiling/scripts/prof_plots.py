...
# median episode = ~456 both-wrist frames (228/wrist), from frames_raw.csv.
# frames is the SIZE-INVARIANT throughput unit (episodes vary 73-461 frames);
# ep/hr * FRAMES_PER_EP = frames/hr.
FRAMES_PER_EP = 456
...
def chart_roofline(indir, outdir, sweep_rows):
    ...
    if obs:
        ...
        # label ABOVE the top bar, not overlapping the bars
        ax.annotate(f"observed plateau\n{obs:.0f} ep/hr",
                    xy=(obs, len(labels)-0.5), xytext=(obs + max(vals)*0.06, len(labels)-0.55),
                    color="crimson", fontsize=10, fontweight="bold", va="top",
                    arrowprops=dict(arrowstyle="->", color="crimson"))
        # note (below-axis) how far the plateau is from the nearest (CPU) wall
        near = min(vals)
        ax.text(0.99, -0.14, f"plateau = {obs/near*100:.0f}% of the nearest (CPU) wall — "
                f"{near-obs:.0f} ep/hr of headroom to CPU alone",
                transform=ax.transAxes, ha="right", va="top", fontsize=9, color="#555555")
    ax.set_yticks(y); ax.set_yticklabels(labels)
    ax.set_xlabel("throughput ceiling (episodes / hour)")
    ax.set_title(f"Roofline: the {obs:.0f} ep/hr plateau vs each resource ceiling\n(observed sits far below EVERY compute/IO wall → soft bottleneck)")
...
def chart_throughput_vs_par(indir, outdir, sweep_rows):
    ...
    ax.plot(par, thr, "o-", color="#1f77b4", lw=2, label="throughput")
    ...
    # size-invariant twin axis: frames/hr (median episode ~456 both-wrist frames)
    ax3 = ax.twinx(); ax3.spines["left"].set_position(("axes", -0.11))
    ax3.yaxis.set_label_position("left"); ax3.yaxis.set_ticks_position("left")
    lo, hi = ax.get_ylim(); ax3.set_ylim(lo*FRAMES_PER_EP/1000, hi*FRAMES_PER_EP/1000)
    ax3.set_ylabel("throughput (K frames / hour)  [size-invariant]", color="#1f77b4")
    ax.set_title("Throughput vs PAR: peak at PAR=64, then regresses\n(right axis: ep/hr is size-dependent; left K-frames/hr is size-invariant)")
    ax.grid(alpha=0.3)
