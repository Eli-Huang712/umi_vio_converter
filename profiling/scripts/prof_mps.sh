...
case "$CMD" in
  start)
    ...
    ;;
  stop)
    # graceful quit first (the ONLY clean way — it tears down the server + context)
    if pgrep -x nvidia-cuda-mps-control >/dev/null 2>&1; then
      echo quit | CUDA_MPS_PIPE_DIRECTORY="$PIPE" nvidia-cuda-mps-control 2>/dev/null
      sleep 2
    fi
    # escalate if anything survives (control/server/bare nvidia-cuda-mps)
    for sig in TERM KILL; do
      pgrep -f nvidia-cuda-mps >/dev/null 2>&1 || break
      pkill -"$sig" -f nvidia-cuda-mps 2>/dev/null
      sleep 2
    done
    rm -rf "$PIPE" "$LOG" 2>/dev/null
    # verify: only zombies (<defunct>) may remain; a LIVE daemon is a failure
    if ps -eo stat,comm 2>/dev/null | grep nvidia-cuda-mps | grep -qv '^Z'; then
      echo "WARNING: a LIVE MPS process survived — restart the container to fully clear" >&2
    else
      echo "MPS stopped + pipe cleaned (any remaining are reaped-pending zombies)" >&2
    fi
    ;;
