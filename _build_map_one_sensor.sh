...
# ---------------------------------------------------------------------------
# P1 direct-feed path (opt-in: UMI_DIRECT_FEED=1). ONE process decodes each
...
# forever (handoff §8). Stock two-process path stays the default below.
# ---------------------------------------------------------------------------
if [[ "${UMI_DIRECT_FEED:-0}" == "1" ]]; then
  driver="${repo_dir}/tool/umi/direct_feed_build_map.py"
